...
const ENERGY_DESCRIPTIONS: Record<string, string> = {
  ...
  'Lowkey': 'Small circles, relaxed vibes, and a Salsa night that feels comfortable rather than loud.',
  'Memory Collector': 'You care about capturing little moments and turning the night into lasting memories.',
  'Figure out yourself': 'You are still discovering your Salsa vibe, and that is part of the fun.'
}
...
const LOOKING = ['Salsa +1', 'New friends', 'A great conversation', 'Salsa crew', 'Just exploring']
...
const LEVELS = [
  { level: 1, min: 0, title: 'Newcomer 💫', unlock: 'Your Salsa Night journey begins.' },
...
export default function Home() {
  ...
  async function authSubmit(e: FormEvent) {
    ...
    if (mode === 'signup') {
        ...
        try {
          ...
          if (error) {
            ...
            if (genderUpdate.error || !form.gender) {
              flash(genderUpdate.error?.message || 'Choose your gender before entering Salsa Night.')
              setLoading(false)
              return
            }
            ...
            setAuthStep('email')
            setAuth(prev => ({ ...prev, otp: '' }))
            flash('Email verified. Welcome to Salsa Night 💗')
          }
        } finally {
      ...
      if (error) {
        ...
        // Salsa Night requires one-time email verification during signup, so do not let an
    ...
    if (error) flash(error.message)
      ...
      setSecretCrushIntroOpen(true)
      flash('Welcome back to Salsa Night 💗')
    }
    setLoading(false)
  }
  ...
  async function deleteAccount() {
    if (!session?.user) return
    const confirmed = window.confirm('Permanently delete your Salsa Night account? This will remove your profile, matches, messages, memories, XP, badges, crushes and all other account data. This cannot be undone.')
    if (!confirmed) return
    setLoading(true)
    try {
  ...
  async function respond(id: string, status: 'accepted'|'declined') {
    ...
    if (error) flash(error.message); else { flash(status === 'accepted' ? 'You have a Salsa +1 ✨' : 'Request declined'); await refreshMatches(); if (status === 'accepted') await loadPeople(session?.user?.id, profile?.college_name || config.college_name) }
    setLoading(false)
  }
      ...
      <div className="chat-list">
        ...
        {accepted.map((m: Match) => (
          <button className={`chat-pick ${activeMatch?.id === m.id ? 'selected' : ''}`} key={m.id} onClick={() => setActiveMatch(m)}>
            ...
            <div>
              <strong>{currentOther(m)?.name ?? 'Unknown'}</strong>
...
function Missions({ missions, completions, progress, openProof }: any) {
  ...
  return <div className="section"><div className="section-head"><div><span className="eyebrow">PLAY, MEET, REMEMBER</span><TooltipHint text="Missions are quick ways to earn XP, level up, and unlock new Salsa Night perks."><h2>Prom Missions</h2></TooltipHint><p className="muted">Level {level} is unlocked. Your newest challenges are below.</p></div><span className="count">{doneCount}/{active.length} active missions completed</span></div>
