...
begin
  ...
  select * into r
  from public.blind_rounds
  where id = round_uuid and challenger_id = uid
  for update;
  ...
  for item in select value from jsonb_array_elements(answers) loop
    begin
    ...
    if qid is null or idx is null then raise exception 'Invalid answer.'; end if;
    if qid = any(seen_ids) then raise exception 'A question can only be answered once.'; end if;
    ...
    if q is null then raise exception 'Question data is unavailable for this round.'; end if;
    if idx < 0 or idx >= jsonb_array_length(q->'options') then raise exception 'Invalid answer choice.'; end if;
  ...
  if score = 3 then
    update public.blind_rounds
      set status = 'success', score = 3, answered_at = now()
    where id = round_uuid;
  ...
  update public.blind_rounds
    set status = 'failed', score = score, answered_at = now()
  where id = round_uuid;

  return jsonb_build_object('success', false, 'score', score, 'expired', false);
end;
$$;
